...
copyplugins: ## copy plugins to test folders
	...
	$(eval TEST_FOLDERS := $(foreach p,$(OS_ARCH), ~/.terraform.d/plugins/gqlprovidertf.com/examplecorp/graphql/2.0.0/$p))
	@sleep 1
	@mkdir -p $(TEST_FOLDERS);
	@for o in $(OS_ARCH); do \
		for f in $(TEST_DESTS); do \
	    	cp ./dist/terraform-provider-graphql_$$o/* ~/.terraform.d/plugins/gqlprovidertf.com/examplecorp/graphql/2.0.0/$$o; \
		done; \
	done
